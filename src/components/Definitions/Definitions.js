...
const Definitions = ({word, category, meanings,lightMode}) => {
    return (
        <div className='meanings'>
            ...
            {
                word ==="" ? (<span className='subTitle'>Escriba la palabra a buscar</span>) : (
                    meanings.map((mean)=>
                        mean.meanings.map((item)=> 
                            item.definitions.map((def)=>(
                                <div className='singleMeaning' style={{backgroundColor:lightMode?'#3b5360':'white', color:lightMode?'white':'black'}}>
                                    <b>{def.definition}</b>
